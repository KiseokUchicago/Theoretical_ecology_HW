...
This week's assignment is to carry out an AIC calculation, and to produce figures that look
like figs. 5 and 6. The data are from the dissertation of Mark Novak, a student of Tim
Wootton. Mark collected time series of mussel densities in the rocky intertidal in New Zealand.
He was interested in determining which of several models best explained his data. The mussels
are attacked by a predatory snail, and part of Mark's work involved comparing plots in which
snails were removed to plots in which snails were not removed. His larger goal was to consider
different models of predation, but considering more complicated models is beyond what we can
manage here.

### Working on filling Given code
```{r}
...
Model<-function(N,pars){
  ...
  for(t in 1:maxt){
  	...
  	Model[t+1] = Model[t]*exp(m*(-TimeDiff))+(I/m)*(1-exp(m*(-TimeDiff)))
...
# understanding model function step by step
# m = pars[1]; I = pars[2];
# Model = numeric();
# Model[1]=N$N[1]
# maxt = length(N$Time) - 1;
...
LHoodNoPred<-function(pars){
	...
	for(plt in 1:MaxPlots){
		...
		SSE = sum((Theory[2:4]-Data3$N)^2) + SSE;  #2.1 Here you update the sum of squared errors between the data and the model
	}
	return(SSE);
}
...
LHoodPred<-function(params){
	...
	for(plt in 1:MaxPlots){
		...
		Data3 = Data2[2:nrow(Data2),];
		SSE = sum((Theory[2:4]-Data3$N)^2)+SSE; #3.4 Update the sum of squared errors
	}
	return(SSE);
}
...
Output2 = optim(par=c(1e-4,1.3,1.3),fn=LHoodPred); #5.1 Use the calculation for the no-predation model as a template here
...
Interpretation of the AIC score:

AIC1: 351.5261  AIC2: 343.8302  Delta AIC: 7.695922 
Lower AIC means better fit. Therefore, predation model better fits the data compared to the no-predation model.
