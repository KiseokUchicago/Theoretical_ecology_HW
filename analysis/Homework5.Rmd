...
t = c(0:(TimeSteps));
# create random variable epsilon with mean 0 and standard deviation of 0.5
eps <- rnorm(Rlzns, mean = 0, sd = StDev)
...
for(j in 1:Rlzns){ #This is a for loop, so that we can plot many realizations
	...
	for(i in 2:(TimeSteps+1)){ #This for loop iterates the model over the number of time steps. 
		N[i] = lambda*exp(eps[i-1])*N[i-1] #2. Change this line so that N[i] is the new value of the population size
	  print(N[i])
	}
		
	Stor[j] = N[i] #3. Change this line so that Store gets the LAST value of N
